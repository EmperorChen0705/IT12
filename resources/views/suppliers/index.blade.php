...
@section('content')
    ...
    @if(auth()->user()->role === 'admin')
        <div class="page-actions mb-3" style="display:flex; justify-content:flex-end; gap:10px; margin-bottom:10px;">
            <button type="button" class="btn btn-primary btn-add-record" data-action="register-supplier">
        ...
        <div class="toolbar-top d-flex flex-wrap justify-content-end align-items-start gap-3 mb-3">
            <div class="search-bar-wrapper" style="flex:1 1 520px;">
                <form action="{{ route('suppliers.index') }}" method="GET" class="search-bar" autocomplete="off">
                    ...
                    @if(request('search'))
                        <button type="button" class="search-clear" onclick="window.location='{{ route('suppliers.index') }}'">
                    ...
                    @forelse($suppliers as $supplier)
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <span style="font-size: 0.75rem; color: #999;">View only</span>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="empty-row text-center">No suppliers found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
