...
@section('content')
    ...
    <div class="metrics-grid"
        ...
        <div class="metric-card">
            ...
            <div class="metric-value">{{ $appointmentsThisMonth }}</div>
        </div>
        <div class="metric-card">
            <div class="metric-label">Avg App / Day</div>
            <div class="metric-value">{{ $avgAppointmentsPerDay }}</div>
        </div>
        <div class="metric-card">
            <div class="metric-label">Services Completed</div>
            <div class="metric-value">{{ $servicesCompletedMonth }}</div>
        </div>
        <div class="metric-card">
            <div class="metric-label">Items Added</div>
            <div class="metric-value">{{ $itemsAddedMonth }}</div>
        </div>
        <div class="metric-card">
    ...
    <div style="margin-bottom:18px;">
        <a href="{{ route('stock_out.index') }}" class="btn btn-secondary"
    ...
    <div class="glass-card glass-card-wide">
        ...
        <form method="GET" style="margin-bottom:12px;">
            <div>
                ...
                <input type="text" name="search" value="{{ $search }}" class="form-input" style="width:100%;"
                    placeholder="Desc / event / subject id">
            </div>
        </form>
        <div style="display:flex;gap:10px;margin-bottom:10px;">
            <button class="btn btn-primary" style="flex:1;display:flex;justify-content:center;align-items:center;"
            ...
            <a href="{{ route('reports.index', array_merge(request()->query(), ['export' => 'pdf'])) }}"
                class="btn btn-secondary" style="flex:1;display:flex;justify-content:center;align-items:center;"
                target="_blank">
        ...
        <div class="table-responsive">
            <table class="table compact">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="empty-row text-center">No activity.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
