...
@section('content')
    ...
    <div class="page-actions" style="margin-bottom:10px;">
        <button type="button" class="btn btn-primary btn-add-record" style="width:100%;display:flex;justify-content:center;"
        ...
        <div class="toolbar-top d-flex flex-wrap justify-content-end gap-3 mb-3">
            <div class="search-bar-wrapper">
                <form method="GET" action="{{ route('stock_in.index') }}" class="search-bar" autocomplete="off">
                    ...
                    @if(request('search'))
                        <button type="button" class="search-clear" onclick="window.location='{{ route('stock_in.index') }}'">
        ...
        <div class="table-responsive">
            <table class="table align-middle">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="empty-row text-center">No stock-in records found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="modal hidden" id="createStockInModal" data-modal @if($errors->any() && old('_from') === 'createStockIn')
            ...
            <form action="{{ route('stock_in.store') }}" method="POST" id="stockInForm" autocomplete="off">
                ...
                <div class="form-row" style="display:flex;gap:14px;flex-wrap:wrap;">
                    <div class="form-group" style="flex:0 0 200px;">
                        <label>Date</label>
                        <input type="date" name="stockin_date" class="form-input"
                            value="{{ old('stockin_date', now()->format('Y-m-d')) }}" required>
                    </div>
    ...
    <template id="stockLineTemplate">
        <tr class="stock-line-row">
            ...
            <td>
                <input type="number" data-name="quantity" class="form-input stock-qty-input text-end" min="1" value="1"
                    required>
            </td>
            <td>
                <input type="number" step="0.01" min="0" class="form-input stock-unit-input text-end" data-unit readonly>
            </td>
