...
@section('content')
        ...
        <div class="toolbar-top d-flex flex-wrap align-items-end gap-3 mb-3">
            <div class="search-bar-wrapper" style="flex:1 1 380px;">
                <form method="GET" action="{{ route('inventory.index') }}" class="search-bar" autocomplete="off">
                    ...
                    @if(request('search'))
                        <button type="button" class="search-clear" onclick="window.location='{{ route('inventory.index') }}'">
            ...
            <form method="GET" action="{{ route('inventory.index') }}" class="d-flex flex-wrap gap-2"
                ...
                <select name="category_filter" class="form-select form-input" style="min-width:180px;">
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat->itemctgry_id }}" @selected(request('category_filter') == $cat->itemctgry_id)>
                            {{ $cat->name }}
                        </option>
                    @endforeach
                </select>
    ...
    <!-- Create Item Modal -->
    <div class="modal hidden" id="createItemModal" data-modal @if($errors->any() && old('_from') === 'createItem')
    data-auto-open="true" @endif>
            ...
            <form action="{{ route('inventory.store') }}" method="POST">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input name="unit_price" type="number" step="0.01" min="0" class="form-input" required
                ...
                <div class="form-row">
                    <div class="form-group" style="flex:1 0 100%;">
                        ...
                        <textarea name="description" rows="3" class="form-input"
                            style="resize:vertical; width:100%;">{{ old('description') }}</textarea>
                    </div>
                </div>
        ...
        <div class="modal-content" style="max-width:820px;">
            ...
            <div style="display:flex;flex-direction:column;gap:18px;align-items:stretch;">
                <div class="glass-card"
                    style="background:#181818;border:1px solid #272727;padding:14px;border-radius:14px;">
                    <form id="catForm" method="POST" action="{{ route('inventory.itemctgry.store') }}">
                        ...
                        <div style="display:flex;gap:8px;margin-top:14px;">
                            <button class="btn btn-primary" id="catSubmit" type="submit" style="width:100%;">Save</button>
                ...
                <div class="glass-card"
                    style="background:#181818;border:1px solid #272727;padding:14px;border-radius:14px;max-height:440px;overflow:auto;">
                                    ...
                                    <td style="text-align:right;">
                                        ...
                                        <form method="POST"
                                            ...
                                            <button class="btn btn-delete"
                                                style="padding:4px 8px;font-size:.55rem;">Del</button>
                                        </form>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" style="text-align:center;opacity:.6;">No categories.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
